...
from config.logs_config import logger_app_type_research_admin


router = APIRouter(
    prefix='/api/v1/admin_panel/type_research',
    tags=['Панель администратора - Раздел тип исследования'],
...
async def type_research_list(
    ...
    if err:
        logger_app_type_research_admin.error(f"ERROR: {err}")
...
async def type_research_details(
    ...
    result, err = await common.get_model_details(model_uuid=type_research_uuid, model_type=TypeResearch, session=session)
    if err:
        logger_app_type_research_admin.error(f"ERROR: {err}")
        raise HTTPException(status_code=HTTPStatus.INTERNAL_SERVER_ERROR, detail=f"ERROR: {err}")
    elif not result:
...
async def create_type_research(
    ...
    result, err = await common.write_model_to_database(model_to_write=type_research_write_model, model_type=TypeResearch,
                                                       session=session)
    if err:
        logger_app_type_research_admin.error(f"ERROR: {err}")
        raise HTTPException(status_code=HTTPStatus.INTERNAL_SERVER_ERROR, detail=f"ERROR: {err}")
